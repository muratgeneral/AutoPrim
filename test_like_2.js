...
async function run() {
    try {
        ...
        console.log("--- Wildcard Test ---");
        const tbl = await pool.request().query(`
            SELECT 
                f.UNVAN,
                CASE 
                    WHEN f.UNVAN LIKE '%F_NANSAL%K_RALAMA%' THEN 'Leasing (Underscores)'
                    WHEN f.UNVAN LIKE N'%FİNANSAL%KİRALAMA%' THEN 'Leasing (N prefix)'
                    WHEN UPPER(f.UNVAN) LIKE '%FİNANSAL%KİRALAMA%' THEN 'Leasing (UPPER)'
                    ELSE 'B2C'
                END AS TestKanal
